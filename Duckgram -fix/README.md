...
| Шаг | Что происходит |
|-----|----------------|
| **5** | Собирает **`lists/telegram-ip-candidates.txt`**: сначала [cidr.txt](https://core.telegram.org/resources/cidr.txt), при блокировке — зеркало GitHub/jsDelivr или **`lists/telegram-cidr-official.txt`** в папке проекта. |
...
| **5** | Обновить `lists/telegram-ip-candidates.txt` (cidr с Telegram / зеркало / `lists/telegram-cidr-official.txt`) |
| **0** | Выход |
...
- **[5]** пробует `core.telegram.org`, затем зеркала (GitHub, jsDelivr) и локальный **`lists/telegram-cidr-official.txt`**; при корпоративном MITM прокси теоретически возможна подмена загрузки (как для любого HTTPS).
...
### Discord (zapret, отдельно от Telegram)

Для **голоса и клиента Discord** правка `hosts` не подходит — нужен обход DPI (например [Flowseal zapret-discord-youtube](https://github.com/Flowseal/zapret-discord-youtube)). В папке **`zapret-discord-youtube-1.9.9a/`** добавлена стратегия **только Discord**:

- запуск: **`discord-only.bat`**
- инструкция: **`zapret-discord-youtube-1.9.9a/STRATEGY-DISCORD-ONLY.md`**
- служба: `service.bat` → Install Service → **`discord-only.bat`**, **Game Filter** выключить; **ipset-all** обновить (п. **7** в `service.bat`)

Duckgram по-прежнему через этот проект (**[4] → [1]**), zapret — для Discord.

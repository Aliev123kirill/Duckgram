...
set /a _g=9 & set /a _t=44
call :pad92_len !_g!
set "_gsp=!_P92!"
call :pad92_len !_t!
set "_tr=!_P92!"
echo   !GRY!^|!R!   !GRN![6]!R!  !WHT!Авто 3-4-5!R!!_gsp!!DIM!просмотр->поиск->обновление!R!!_tr!!GRY!^|!R!
...
if not "!MODE_AUTO!"=="1" call :wait
if "!MODE_AUTO!"=="1" goto :auto_after_5
set "MODE_AUTO="
goto :menu
:up_no_script
...
goto :menu
:auto_after_5
rem Шаг [1]: автоматический выбор пункта «Включить», затем создать маркер .duck и закрыть окно.
call :enable
if "!AUTO_QUIT!"=="1" exit /b 0
goto :menu
:autodetect
call :cls_ui
...
echo.
echo   !WHT!Готово.!R!
if not "!MODE_AUTO!"=="1" echo   !DIM!  Нажмите любую клавишу, чтобы вернуться в меню...!R!
rem См. :wait_any_key (timeout + сброс буфера клавиш).
if not "!MODE_AUTO!"=="1" call :wait_any_key
if "!MODE_AUTO!"=="1" goto :auto_after_4
goto :menu
:ad_no_detect
...
if not exist "%DUCK%" >"%DUCK%" echo duck
rem Автозакрытие после успешного включения: короткая пауза, чтобы показать результат.
echo.
echo   !DIM!Окно закроется автоматически...!R!
timeout /t 3 >nul
exit /b 0
:en_no_cfg
call :cls_ui
...
if not "!MODE_AUTO!"=="1" call :wait
if "!MODE_AUTO!"=="1" goto :auto_after_3
goto :menu
:hosts_echo_row
set "_hl=0"
